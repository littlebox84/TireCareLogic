...
require('../src/catalog-engine.js');
require('../src/research-catalog.js');
require('../src/catalog-runtime.js');
...
assert.equal(C.search('TR413').valves[0].maxPsi,65);
assert.equal(C.search('600HP').valves[0].maxPsi,80);
assert.equal(C.search('TR501').valves[0].maxPsi,null,'TR501 must not invent universal PSI');
const audit=C.audit();
